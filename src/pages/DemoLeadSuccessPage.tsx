...
import {
  ...
  buildDemoLeadWhatsAppUrl,
  buildWhatsAppMessage,
  logDemoLeadClientError,
  markDemoLeadWhatsAppClicked,
  type DemoLeadValues,
} from '../lib/demoLead';
import { getSalesWhatsAppNumber } from '../lib/whatsapp';

const whatsappTrackingTimeoutMs = 1500;
...
function readStoredLeadTracking(): { leadId: string; trackingToken: string } {
  try {
    return {
      leadId: window.sessionStorage.getItem(DEMO_LEAD_ID_STORAGE_KEY) ?? '',
      trackingToken: window.sessionStorage.getItem(DEMO_LEAD_TRACKING_TOKEN_STORAGE_KEY) ?? '',
    };
  } catch {
    return { leadId: '', trackingToken: '' };
  }
}

export default function DemoLeadSuccessPage() {
  const [lead] = useState<Partial<DemoLeadValues>>(readStoredLead);
  const [tracking] = useState(readStoredLeadTracking);
  const [isOpeningWhatsApp, setIsOpeningWhatsApp] = useState(false);
  const whatsappUrl = buildDemoLeadWhatsAppUrl(getSalesWhatsAppNumber(), lead);
  const whatsappMessage = buildWhatsAppMessage(lead);

  const trackWhatsAppClick = async () => {
    if (!tracking.leadId || !tracking.trackingToken) {
      return;
    }

    const controller = new AbortController();
    const timeoutId = window.setTimeout(() => controller.abort(), whatsappTrackingTimeoutMs);

    try {
      await markDemoLeadWhatsAppClicked(tracking.leadId, tracking.trackingToken, controller.signal);
    } catch (error) {
      logDemoLeadClientError('Demo lead WhatsApp tracking failed.', error);
    } finally {
            ...
            <div className="mt-8 flex flex-col gap-3 sm:flex-row">
              {whatsappUrl ? (
                ...
                >
                  ...
                  {isOpeningWhatsApp ? 'Abrindo WhatsApp...' : 'Continuar no WhatsApp'}
                </a>
              ) : (
                <button
                  type="button"
